...
- `packages/analyzer/` — React/Vite SPA (**v3**). Google OAuth login, semester switcher, cohort list, per-submission drill-in (overview / timeline / replay / validation / export / source), a 26-flag heuristics tuning UI (per-flag **weight** 0.0–2.0 + on/off — _not_ the heuristics' own thresholds, which live in `analysis-core/heuristics/config.ts`; the 26 = 18 per-submission event-stream heuristics + 6 validation-derived integrity flags + 2 cross-submission heuristics, see `analysis-core/heuristics/known-flag-ids.ts`), cross-flags view. Also a standalone `/local` route that runs entirely in-browser (drop a `.zip`, no server). UI on top of `analysis-core`. Note: the submission **Export tab is a v3.1 stub**; the working findings export (markdown + PDF) exists only under `/local`.
