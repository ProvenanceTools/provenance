...
export const nodes: Record<string, ArchNode> = {
  ...
  r_replay: {
    title: 'Replay tab',
    body: 'Replay reconstructs a file as it stood at a chosen moment by applying the recorded deltas forward to a target global index. When the caller names no point, the server resolves a sensible default (the last doc.save for that file, or failing that the last event in the stream) from the same index it just parsed, so "show me the final state" costs no extra query.\n\nEvery reconstruction is served with a short private cache header, which is the client-side staleness bound: the server-side reconstruction cache is process-local and never explicitly invalidated, so a brief TTL is what stops a superseded view lingering.\n\nA submission with more than one contributor renders one lane per contributor on a single shared playhead (`?split=0` opts out; off for a solo submission). Lanes are a presentation of the one ordered stream fetched above, never a second query or a filter: they add a contributor to the view and can never remove one, which is the only reason this mode is allowed where ContributorSelect\'s seek-only control is not. The grid groups lanes by each contributor\'s active file, so two contributors sitting in the same determinate file collapse to one converged pane rather than two panes of identical bytes; a file with no single content at the playhead still renders the existing refusal, `AmbiguousFilePanel`, now sized to its group\'s columns inside the lane grid instead of the full pane. The transport gains one activity ribbon per contributor in the same index space as the seam ticks, with a band where two or more overlap labelled "both recording" — an honest, cheaper fact than "unordered", which is decided per file at a specific playhead and is not something the ribbons can know about the whole timeline.',
    links: [
  ...
  r_src: {
    title: 'Source tab',
    body: 'The tab that has to work without the thing it displays. Submitted source bytes are stripped at ingest, so nothing here is read from stored file content. The file list and per-file verdicts are derived from the signed manifest compared against the on-disk hashes recorded in the event stream; because the manifest is signature-verified, the code trusts its sha for present files rather than re-hashing bytes that are gone. A file the manifest carries with role attachment gets a fourth verdict rather than match or mismatch: it was sealed and hashed but never captured, so there is no event history to compare it against, and check 8 can only say whether its present bytes agree with their own signed sha — a disagreement there is real, detectable tampering, reported even though the file was never meant to be reconstructed.\n\nThe content shown is reconstructed by replaying the log to the end — for a reviewed file; an attachment has no reconstruction to show at all — and the tab now refuses to overclaim what that reconstruction is. Its lede says in as many words that this pane is not the file that was submitted; for a file whose verdict is match it says only that the submitted code is EXPECTED to be identical to what is shown, because the hash agreeing is a statement about the last state the recorder observed on disk, not a byte-for-byte proof that the replay reproduced it. Reconstruction fidelity is not itself verified anywhere, so asserting equality would have been the tab quoting a guarantee no code makes. For a mismatch the pane is the recorded final state, which by definition differs from what was handed in.\n\nThe chain-integrity gate that decides whether any of those verdicts may be trusted is now READ rather than re-derived. getStoredChainIntact selects validation_results.check_3_status and passes a { chainIntact } gate down into submittedFileVerdicts; the tab used to run its own live runValidation(bundle) over the re-parsed blob, which is a second implementation of a question the ingest pipeline had already answered and stored. Two implementations of "did the chain verify" are two answers waiting to disagree, and the one a grader reads must be the one the validation tab shows.',
    invariant:
