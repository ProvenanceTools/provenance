...
export const nodes: Record<string, ArchNode> = {
  ...
  fanout: {
    title: 'Fan out',
    body: 'The unit is an assignment scope times a submitter, not a folder. A folder is walked for every .provenance/ at any depth, and each one is synthesized into its own flat bundle carrying that directory’s own relative paths, so a cloned 61B repo holding proj2/ and lab5/ becomes two independent submissions rather than one unusable archive. Scopes are self-identifying — the manifest carries assignment_id and semester — so nothing has to be told where to look. What ingest_scope adds is a DECLARED SUBMISSION TYPE for the batch: self_identifying (the default) accepts every sealed scope; bundle_zip demands exactly one scope at the tree root; repo_whole keeps only the repo root and excludes nested scopes rather than fanning them out; repo_scoped selects scopes by path_glob. The declaration comes from the assignment row or, beating it, from a per-request override on the ingest call — the override is simply a resolver that ignores its assignment key, so nothing below the entry point knows which of the two it got, and every one of the three entry points now offers it. An override also short-circuits the per-assignment lookup entirely, because a declaration about the whole batch leaves nothing per-assignment to read.\n\nThat declaration is what makes a batch checkable for homogeneity. A submission whose shape does not match fails as submission_type_mismatch, and so does a repo_scoped glob that selects nothing — otherwise a typo’d glob drops a whole cohort while the ingest reports success. The failure is per SUBMISSION, never per batch: the export is streamed folder by folder and staged incrementally with no transaction spanning it, and rows are never deleted, so aborting mid-batch would strand a half-ingested one with no way back. A heterogeneous batch therefore shows up as a pile of these entries in the same skipped array as no_seal and scope_excluded — the existing channel, not a new one. That array is durable rather than transient. The single-shot route ingests inside the request and can inline it in its own response, but the chunked upload answers 202 long before staging runs and cannot, and finalize is no help either: the job summary is nothing but a count of ingest_files rows, and a skipped scope has no row — that is what being skipped means. So the stager persists the list to ingest_jobs.skipped before it opens the finalize gate, and GET /ingest/jobs/:jobId serves it back. Both upload mechanisms go through the same mapper into the same column, so a reader cannot tell which one was used, and provgate does not have to know. The column is nullable with no default because null and empty are different answers: null means resolution has not finished — still staging, or aborted part-way — and an empty array means it finished and skipped nothing. Collapsing those two is how the chunked path used to report a heterogeneous batch as clean. A scope counts as sealed by either shape: the classic manifest.json, or the per-session manifest-<session_id>.json the recorder rewrites on every checkpoint, which is the only seal a git submission ever has. Recognising just the classic name here discarded every rolling-sealed scope as no_seal and made git submission produce nothing at all. no_seal now means a .provenance/ that genuinely nothing signs, and it is still reported against its scope rather than dropped. Then each scope’s bundle is shared by every co-submitter of a group submission, and each of them gets their own ingest_files row and their own queued job — but, since migration 0029, ONE submission between them: the first to be processed creates it and the rest attach to it as contributors, which is why the Gradescope path no longer narrows its dedup key by student. The rebuild that produces those bytes is byte-deterministic, because its sha256 is what dedup keys on — and a flat folder with a single root .provenance/ still yields exactly one scope with byte-identical entries, so the classic Gradescope path is unmoved.\n\n"Bounded" is two independent limits. The stager keeps at most INGEST_STAGE_CONCURRENCY per-bundle tasks in flight (default 1, exactly serial) with backpressure, so the producer cannot run ahead of the writers; the worker separately claims batches of INGEST_CONCURRENCY files. "Cancellable" is cooperative: cancelling does not remove queued jobs from pg-boss, it sets ingest_jobs.status, which every file job re-reads before doing any work and then marks its still-pending row discarded. One gate covers queued, in-flight and restart-replayed jobs alike.',
    invariant:
