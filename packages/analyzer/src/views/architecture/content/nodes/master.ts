...
export const nodes: Record<string, ArchNode> = {
  ...
  sig: {
    ...
    body: 'The recorder records document opens, changes, saves and closes, selection and focus changes, pastes, external file changes, terminal commands, git state changes, periodic extension snapshots and heartbeats, for any file inside the assignment root, not only the files under review. The narrower list matters in one place only: the expected-content model and its file watchers cover files_under_review, because those are the files whose external modification is evidence.\n\nSome of that list is now a course’s to narrow. The manifest carries a signed capture policy, and the recorder checks it at a single choke point inside emit, so a course can switch off selection changes, focus changes or terminal capture and lengthen the heartbeat cadence. It cannot go further: every other kind here is on a hard floor that the policy schema has no key to express, because reconstruction, validation and the exculpatory heuristics depend on them.\n\nWhat is excluded outright is as deliberate as what is included. Documents outside the assignment root are dropped, non-file URI schemes (untitled buffers, output panels, git overlays) are dropped, and the recorder’s own artefacts (everything under .provenance/ and the activation manifest) are dropped. That last exclusion is not tidiness: a student opening the live log would otherwise trigger a feedback loop in which each append re-enters as a document change and appends again.\n\nOne kind in the vocabulary is produced only by the VS Code recorder so far. peer.observed records that a foreign .provenance/ log was seen in the working tree — its filename, byte digest, and the foreign chain\u2019s highest seq and final hash — written into the observing session\u2019s own signed chain. It is how deleting a partner\u2019s log leaves evidence that it existed, and it is the only thing that can tell a rolling seal whose log was never pushed from one whose log was removed. Note it does not contradict the .provenance/ exclusion above: that rule drops the directory as DOCUMENT content, while a witness reads a foreign file\u2019s bytes to hash them and never re-enters as an edit. The readers landed first, deliberately \u2014 the type, the narrowing, the conformance vectors and the reconciliation \u2014 and the directory watchers followed, which is the migration ordering the format rule requires. The VS Code recorder now runs one watcher over .provenance/, whose callbacks only enqueue a filename; the reading, hashing and chain-tip parsing happen on the checkpoint cadence, so the sub-millisecond handler budget is untouched and at most one observation per file is written per interval. It never renames, rewrites or deletes a foreign log \u2014 recording state unparseable is the entire response to one it cannot read, which is the lesson of a startup recovery that once quarantined a partner\u2019s evidence and let git blame the victim. The two sibling recorders port from it.',
    links: [
  ...
  gs: {
    title: 'Gradescope',
    body: 'Gradescope unzips whatever a student uploads. A bundle submitted there does not survive as a .zip. It arrives in the staff export as a folder of loose files per submission, under one top-level directory alongside a submission_metadata.yml that names the submitters.\n\nThat is why the server cannot forward what Gradescope hands it unchanged. The Gradescope ingest path locates the metadata, rebuilds a flat bundle zip from each submission folder, and stages one ingest row per submitter. A group therefore fans out into N rows over one artifact, and what reassembles them into ONE submission with N contributors is no longer their bytes happening to agree: since migration 0033 every row also carries source_group_key, the declared <folderKey>/<scopePath>, and dedup asks that before it asks the blob hash. The rebuild is still byte-deterministic, and the rebuilt archive’s sha256 is still the second key — but it is no longer the only thing standing between a declared partnership and two submissions of one repository for the cross heuristics to compare.',
    links: [
      {
  ...
  dedup: {
    title: 'Dedup — declared group, then content hash',
    body: 'Before any heavy processing, ingest recognises a bundle it has already seen. Two keys, asked in order: a declared group, (semester_id, source_group_key), and then the bytes, (semester_id, blob_sha256). Because both checks are cheap and happen first, re-sending an unchanged bundle costs almost nothing.\n\nThe order is the interesting part. The group key is asked first because bytes can diverge and a declaration cannot: two co-submitters of one Gradescope group used to be collapsed only because the fan-out gave every row identical bytes, which was a property of one function rather than an invariant, and any divergence split a declared partnership into two submissions of one repository — exactly what the cross heuristics fire on. When both keys would fire they name the same submission; a file with no declared group takes only the blob probe, as it always did.\n\nThat cheapness is also what lets provgate treat its watermark as an optimisation rather than a correctness mechanism: if the watermark is wrong, dedup still prevents duplicate submissions.',
    invariant:
      'Dedup runs before any heavy processing, never after — and the declared group is asked before the bytes.',
  ...
  cross: {
    ...
    body: 'Two heuristics compare submissions with each other. The first groups pastes of at least a hundred characters across bundles by content identity, joining a group on either an exact sha256 match or a high line-overlap ratio, one group covering both mechanisms, because splitting exact and fuzzy into separate flag types fragments what is really one finding. The second compares editing rhythm: Jaccard similarity over the set of 3-grams of each bundle’s event-kind stream, which is insensitive to content and therefore says nothing about what was written, only about how.\n\nThey run on a different schedule from everything else in this lane. Per-submission analysis belongs to one file’s job; a cross-submission comparison is meaningful only across a cohort, so ingest finalization enqueues a separate semester-scoped sweep, collapsed by singleton key so a batch of a hundred files produces one recomputation rather than a hundred.\n\nThe job is semester-scoped; the comparison inside it is not. Submissions are grouped by assignment and each group runs alone, with bundles loaded only for the assignments that have two or more — because a pool selected on semester alone compared a student’s own hw1 against their hw2, and a helper carried forward into the next assignment came back as a high-severity content-sharing finding naming the same student twice. Within an assignment, two bundles that share an observed commit — or, since D18a, a shared signed session — are not compared at all: a git-native group submission carries both partners’ signed logs in one add-only .provenance/, so without that exclusion every honest pair matched on every paste. The session key exists because the commit key is a proxy for exactly that, and git observation is an optional capability: a shared .provenance/ yields no observed commits when the host exposes no git integration, when the partners commit from a terminal outside their sessions, or when there is no git at all. It carries its own starter guard, a cohort-fraction ceiling that lets a session key move only a strict minority of the pool, so a staff-recorded skeleton committed into .provenance/ cannot switch detection off course-wide. All these narrowings subtract comparisons; none adds a finding, and the withheld ones are stated in the exclusion register rather than silently absent.',
    links: [
      {
  ...
  enroll: {
    title: '/enroll',
    body: 'The only route in this analyzer where a student rather than course staff WRITES anything, and the only way a student obtains a credential. It is no longer the only student-reachable route: /faq is student-facing too, but it is read-only and needs no account, so this remains the single point where a student\u2019s browser talks to the server. It is guarded by RequireAuth and nothing else: no RequireStaff, because a student has no memberships row and would be bounced straight to /home, and no app shell, because the staff chrome is navigation they cannot use. A student session gets a 200 from /me with an empty memberships array, and that is the whole entitlement.\n\nThere is no course and no semester in the path, and that absence is the design rather than an omission. The predecessor posted to a semester-scoped route and needed a semester uuid in the query string; a 2.1 credential names an institution and nothing else, so one credential serves every course the student will ever take, forever. The error handling reflects it structurally — there is no roster branch and no not-found branch, because the path carries no id that could be missing.\n\nIt is a PASTE, not a keypair generator. The master secret and the private key are generated by the recorder and never leave the machine; the page takes the public half the student reads out of their editor, posts it to the credential endpoint, and hands back the issued credential as a one-line paste to import. There is no code path here that generates, receives or transmits private key material, and there must never be one.\n\nThe hazard the page is built around is that a master secret and a public key are both sixty-four lowercase hex characters, so a student who copies the wrong line ships their private identity to the server. Two partial defences, both necessary. The recorder marks an exported secret with a provenance-secret-v1: prefix and the page refuses anything carrying it — checked before extraction, not after — and a prose heuristic catches a student who pastes the surrounding document without the marked line. A bare secret with the marker stripped remains indistinguishable, and the page says so in an amber warning rather than pretending otherwise.',
    invariant:
      'Institution-scoped: no course, no semester, one credential forever. And no private key material touches this page, in either direction.',
    links: [
      {
        label: 'EnrollView.tsx',
        href: `${GH}/packages/analyzer/src/views/enroll/EnrollView.tsx`,
      },
      {
        label: 'credential.ts (server route)',
        href: `${GH}/packages/server/src/api/v1/routes/credential.ts`,
      },
    ],
  },
  faq: {
    title: '/faq',
    body: 'The student FAQ, and the second of the two routes a student can reach. It differs from /enroll in the way that matters: it is read-only and entirely unauthenticated. There is no RequireAuth, no RequireStaff, no app shell, no query, and no server call of any kind \u2014 the page is copy, and a signed-out visitor on a phone renders the whole of it.\n\nThe absence of auth is the design rather than an oversight. Students have no analyzer account, and the OAuth hd check that guards every other route would turn away precisely the audience the page exists for. That places it alongside / (the landing page) as one of two auth-free surfaces, and it is the reason a course can put the URL in an announcement, a syllabus, or a lecture slide without anyone hitting a sign-in wall.\n\nThe copy lives in faq-content.ts as structured data rather than as markup, and that module is the single source of truth: docs/student-faq.md was a second copy of the same words until it became a pointer, because the two had already begun to drift. Sections, the contents rail, and every per-question anchor derive from that one array, so a question cannot exist in the nav and be missing from the page.\n\nThe per-question ids are the part to handle carefully. Staff link a student to a single answer rather than the whole page, so an id such as q-own-paste ends up pasted into course announcements and Ed posts. They are published URLs in every sense except that nothing in the build enforces it, so renaming one silently breaks links already in circulation.\n\nStyling follows the landing page rather than the staff chrome \u2014 light surface, gray-50 ground, orange-700 accent \u2014 because a student arriving cold should land on something that looks like the public face of the product, not like a triage console they have wandered into.',
    invariant:
      'Public, read-only, and account-free: no RequireAuth, no server call, no query. The per-question anchor ids are published URLs \u2014 renaming one breaks links already in circulation.',
    links: [
      {
        label: 'FaqView.tsx',
        href: `${GH}/packages/analyzer/src/views/faq/FaqView.tsx`,
      },
      {
        label: 'faq-content.ts (the copy, as data)',
        href: `${GH}/packages/analyzer/src/views/faq/faq-content.ts`,
      },
    ],
  },
  compose: {
    title: '/compose/manifest',
    body: 'The staff manifest composer: build a .provenance-manifest, sign it with the course private key, and download it into the assignment repository. It closes the loop this plate opens with, because the manifest it produces is precisely the artifact the workspace gate checks before the recorder will record anything.\n\nSigning happens in the browser, and the course private key never leaves the machine. That is not a convenience — the course key is deliberately offline, which is the entire reason a server-held enrollment subkey exists at all. A server that could sign manifests could weaken any course’s capture policy, and the capture policy is signed precisely so a professor can turn capture down while a student cannot turn it off.\n\nThe mechanics are stricter than the promise. The key is chosen with a file picker and the component never reads the file except inside the sign handler — not on selection, not to preview the public half, not to check it against the certificate; the retained state is a File handle, and a handle is not key material. Inside the handler the text is read, passed straight to the composer, and dropped on return: a parameter and a local, never state, never a ref, never a closure that outlives the call. Derived bytes are zeroed in a finally and only the public half is returned. The module imports no API client and touches no storage, no cookies and no console. A dedicated leak test drives a real signing run and asserts the key hex appears in no request, no storage write, no log argument, no beacon, no produced blob, and no DOM node or attribute.\n\nThe certificate is uploaded by staff rather than fetched, and the root public key comes from the browser build or is pasted, mirroring the CLI flag. Without a root key the page refuses to produce a 2.0 manifest at all rather than emitting one nobody can anchor.\n\nBecause this is signed-format code living in a UI package, correctness is pinned by composition rather than by unit tests: a gate under tools/ spawns the real sign-manifest.ts as a subprocess and compares octets, at 2.0 and at 1.0, including the trailing newline and including the signature itself. It runs under the tools suite, which is not part of the workspace test run.',
    invariant:
      'The course private key is read inside the sign handler and dropped on return. It is never state, never a request, never storage — and the output is byte-identical to the CLI.',
    links: [
      {
        label: 'ManifestComposerView.tsx',
        href: `${GH}/packages/analyzer/src/views/compose/ManifestComposerView.tsx`,
      },
      {
