...
export const nodes: Record<string, ArchNode> = {
  ...
  t2: {
    title: 'Timeline',
    body: 'The tab pages the events endpoint to exhaustion and builds the same EventIndex the offline route builds from a zip, then mounts the same component. One implementation of "browse a bundle’s events" serves both paths, so a filter or a detail pane written once works in both.\n\nWhat the two routes can CLAIM about order used to differ, and no longer does. /local parses the archive, so it could call reconstructionScopeFor(bundle, index) and get the happens-before relation; the server-backed tab pages EventRows and never sees a bundle, so it passed no scope at all and orderTimelineEvents fell back to the index’s own globalIdx — which is WALL-derived, and across two machines can contradict a proven happens-before, with no break markers to warn a reader. Both tabs now get a real ReconstructionScope. The server-backed one is built by useServerScope from the paged index plus SubmissionSummary.contributor_stamp, the server’s own establishBundleContributors verdict projected onto the wire; sessionsFromIndex recovers the per-session shape the observed DAG and the relation read, and neither reads a hash, so nothing extra is fetched.\n\nOne hook rather than two, deliberately: Replay needs the identical input, and two hooks would be two chances to disagree about who recorded what for one submission, on two tabs a grader flips between. Cost is unchanged for the ordinary case, because buildReconstructionScopeFromSessions builds no DAG and no reachability closure for a scope without two provably different contributors and returns the same ordering-null scope that preceded it. An older server that sends no stamp reads as unstamped and lands in exactly that place — failing toward today’s behaviour rather than toward a refusal to answer.\n\nIt used to be a bespoke list that sliced the first five hundred rows out of a query already capped at two thousand. On a large submission that silently showed a fraction of the stream, and the missing part is exactly where someone hunting an anomaly would be looking. The current ceiling is far higher and, more importantly, is surfaced as a visible error when it is hit: for this product a truncated timeline is strictly worse than a refusal, because a refusal cannot be mistaken for a quiet session.',
    invariant:
    ...
    links: [
      ...
      { label: 'useServerScope.ts', href: `${GH}/packages/analyzer/src/data/useServerScope.ts` },
    ],
  },
  jump: {
