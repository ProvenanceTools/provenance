...
export const nodes: Record<string, ArchNode> = {
  ...
  split: {
    ...
    body: 'One enumeration of the overlapping session pairs in a bundle, one bounding rule for a session that has no session.end, and now TWO decisions taken in the same loop: are the two sides the same person, and if so, were they the same machine. It returns a three-way partition rather than a filtered list — judged pairs go to the multiple_sessions_overlap heuristic, collaboration pairs and multiMachine pairs go to the coverage stage — and every overlapping pair lands in exactly one of the three, by construction, because there is one loop and one push per pair.\n\nThe second decision exists because the flag accused a supported flow in its opening clause and conceded it in its closing one: it said the overlap indicated clock manipulation or log forging, and then, for the proven-same-contributor case, that the remaining innocent explanation was somebody recording on two machines under one identity — which is D5, a first-class flow where each machine enrols independently with its own keypair and the shared student_ref groups them into one contributor. The evidence separating the two was already in the bundle and was being discarded. student_ref is per PERSON; the long-lived student_pubkey the chain walk returns is per MACHINE, because a second enrolment mints a fresh student keypair over the same ref, which is exactly what the credential minter counts as a machine. No format change, no new event field, and identity/ is untouched. The adjacent key is a trap worth naming: session.start also carries session_pubkey, which is per-session ephemeral, so keying on it would have suppressed every same-contributor overlap including the single-machine clock manipulation the heuristic exists for. Two 64-hex fields on one payload, one per machine and one per session.\n\nThe contributor is NOT split. attributedContributorKey still keys on student_ref alone, so D14’s per-contributor scoring, the submission_contributors cut-over and the sole-contributor rule all keep seeing one person; splitting them would split that person’s score across two apparent people, which is what migration 0029 exists to prevent. Only the overlap JUDGEMENT gained sight of the machine.\n\nThis exists because the alternative had already been built twice. The heuristic suppressed a two-verified-people overlap with a bare continue taken before the flag’s detail object existed, and run returns nothing but flags, so the strongest exculpatory evidence in the system was computed and discarded. The panel that wanted to show it therefore recomputed the overlap itself, in the analyzer, and defended the duplicate with a test proving the two implementations agreed. The test was good and the duplicate was still wrong: two implementations that agree today drift later, which this repo has already paid for as "26 versus 25 flags" and "21 versus 25 tables".\n\nWhat prevents the drift now is the type system, not a comparison test, and the three-way split strengthened it rather than weakening it. JudgedOverlap became a discriminated union whose comparison is typed same_machine | unknown, so neither a two-person pair nor a two-MACHINE pair has an arm to arrive in, and re-adding a local comparison === "different" check is still a TypeScript build failure rather than a silent divergence. The same_machine arm carries attributed contributors, so the flag that names a person is typed to have proof of who — the old runtime fallback to "the same contributor" is gone rather than merely unreachable. CollaborationOverlap likewise carries its two contributors already narrowed to the attributed arm, so an unverifiable or unattributed session cannot appear on the collaboration side at all: the unverifiable-is-not-unattributed rule is enforced by the compiler instead of by care. The partition property itself — disjoint, and exhaustive over the overlapping pairs — is asserted directly, which is a smaller and stronger claim than "the two copies still match".\n\nA same-machine overlap and a two-machine overlap are kept as different facts in different arms because folding either way loses something. Folding two machines into judged accuses the supported flow; folding one machine into multiMachine loses the detection. And two machines is not two PEOPLE, which is why it is a third arm with its own coverage field rather than more collaboration: rendering it as "two contributors recorded concurrently" would tell a grader that two students collaborated when one person moved between their own desks.\n\nThe contributor comparison is three-valued and is never a string compare on the contributor key. Every unattributed session carries its own singleton key, so comparing keys would read "unproven" as "different people" and delete the finding through the back door.',
    invariant:
      'One enumeration, two decisions, three parts. A pair the heuristic must not judge — two people, or two of one person’s machines — has no arm to arrive in.',
    links: [
      {
  ...
  coverage: {
    ...
    body: 'Everything the analysis established that is not a finding. Concurrent recording by two verified, distinct contributors; one verified contributor recording on two of their own enrolled machines; how many sessions came out attributed, unverifiable and unattributed, and whether a root key was configured at all; how much of the commit graph was observed, and what the DAG could not read; whether any session ever named a repository; how many bytes and checkpoints sit past each rolling seal; which logs the loader had to truncate a torn trailing line off; and which artifacts it dropped entirely. Facts, in the neutral palette, never scored and never ranked.\n\ntornTails and droppedArtifacts sit beside each other and are never merged, because they are different claims and the panel’s two sections say so: "Interrupted writes" reports a session that WAS analysed, minus a half-written last line, while "Files not analysed" reports a file left out altogether. Folding them would tell a grader that a session they can see in full was excluded. concurrentRecording and multiMachineRecording are kept apart for the mirror-image reason: rendering one person’s two enrolled machines as concurrent recording would tell a grader that two students collaborated when one person moved between their own desks.\n\nIt exists because a system that only ever emits findings can only ever say worse things about a student. An overlap between two proven partners is the assignment being done as assigned, and it was previously computed and thrown away. A partner’s log that could not be opened is a fact about the archive, not about the person. A deployment with no root key produces a page of unverifiable sessions that mean "we could not check", and saying so is the difference between a misconfiguration and a cohort-wide integrity finding.\n\n"Always visible" is taken literally, and it means answering in all three states, because silence and "nothing to report" are different claims. With no parsed bundle the panel says NOT AVAILABLE — it must never zero, because a zeroed panel asserts no commits observed, no contributors and no root key, which is a stronger statement than the truth and a false one. With a bundle and nothing to note, it says so. With facts, it shows them. Its home is the submission, above the verdict surfaces on both overview surfaces, because dropped artifacts and "no root key configured" describe the submission rather than the replay; it shipped inside the Replay tab, collapsed, which failed "always visible" twice over.\n\nOne predicate here is easy to get subtly wrong, and was. repositoryAssumedSingle asks whether any observation is folded into the ASSUMED_SINGLE_REPOSITORY sentinel, and the obvious spelling — "no discriminator was recorded anywhere" — answers false for a MIXED scope, where part of the graph is labelled and the rest is still in the sentinel. That is precisely the scope where the caveat matters most, so the predicate is membership in the repository list rather than the absence of a discriminator.\n\nThe stage is pure and isomorphic, so one function serves both surfaces. /local computes it in the browser from the bundle it has already parsed; the server serializes it onto the submission summary from the call site that already holds the parsed index, so nothing new is computed and no extra parse is paid for. The wire shape is deliberately the flat CoverageFacts aggregate and never BundleContributors, whose bySession is a ReadonlyMap and would serialize to an empty object without complaining.\n\nThe no-facts branch survives that and now means something narrower: not “this view cannot compute them” but “this server did not send them”, which today can only be a server older than the field. It is still not a statement about the submission, which is why the panel says that in those words rather than rendering zeroes.',
    invariant:
  ...
  capab: {
    title: 'The capability reader',
    body: 'One reader for the three §5.6 capability reports, walking every session.start in the bundle and narrowing each field independently. Every read returns three answers, not two — absent, recorded, malformed — and the third is what makes the second safe. git_capture and witness_capture are closed enums, so a value outside the set is a nonconforming writer rather than a meaning, and it never reaches a staff-facing surface as though it were one. The narrowing itself lives in log-core rather than here, for the same reason peer.observed’s and git.event’s do: four consumers need identical rules, this repo’s analyzer and server through analysis-core, plus the two sibling recorder repos, which need them in order to EMIT conformant payloads.\n\nAbsent is not unavailable, and that is the whole discipline of the module. 1.x support is permanent, so absence is the permanent ordinary answer and is carried by every bundle recorded before the fields existed — which is every bundle in existence at the moment they landed. A reader that rounded absence up to "witnessing was impossible here" would manufacture a coverage claim about the entire archived corpus.\n\nThe bundle-level summaries therefore require unanimity to say impossible. One session reporting that git was unavailable to it does not establish that the scope had no git observation — a partner’s session may have had it — so impossible needs no session reporting available AND every session reporting at all, and a single unreported session takes the answer to unknown. The failure direction is toward not knowing.\n\nWhat consumes it today is the witnessing reconciliation. reconcileWitnesses gains witnessingCapability at bundle level and a capability on each session’s coverage, and that per-session value sits on a DIFFERENT AXIS from state: state says whether anyone witnessed this log, capability says whether this log’s recorder could have witnessed anyone. unwitnessed stays blameless in all three; what changes is what a surface is entitled to say about it. git_capture has its own bundle summary and an observation-gap helper beside it; file_scope has neither, because a scope answer is per-path rather than per-bundle — wasFileWatched returns watched, not_watched or unknown for one path, and the union list it also exposes is for display, never the basis of that answer. That consumer is itself not yet wired into the pipeline, so this reader ships ahead of anything a grader can see — the same readers-before-writers ordering Manifest 2.0, the rolling seal and peer witnessing each took.\n\nThe scope list is validated whole or not at all. A single entry that is absolute, carries a colon outside a drive letter, or contains a .. segment rejects the ENTIRE set, because dropping only the offender would hand a consumer a silently narrowed list that then says "this file was not watched" about a file that was. That is validatePeerObservedPayload’s rule, for exactly the same reason. And nothing here is ever a finding: no Flag, no ninth check, no severity, no score.',
    invariant:
      'Absent is not unavailable. And a bundle may only say “impossible” unanimously — one unreported session takes it to unknown.',
    links: [
      {
        label: 'capability/session-capabilities.ts',
        href: `${GH}/packages/analysis-core/src/capability/session-capabilities.ts`,
      },
      {
        label: 'session-capabilities.ts (log-core)',
  ...
  hi: {
    ...
    body: 'These four are behavioural inferences about the log’s structure, which is what separates them from the validation checks upstream: a broken chain is proof, an overlapping session pair is an argument. They sit beside the cryptographic findings in the UI because a reviewer wants one list, but their confidences are 0.8, 0.75, 0.9, and — for the overlap, which now answers at two strengths depending on whether the evidence settled the question — either 0.95 or 0.5, rather than 1.0.\n\nTwo of them encode a hard-won negative result. gap_in_heartbeats fires only when at least one other event was recorded strictly inside the gap: an empty gap is a suspended machine, not a paused recorder, and before that rule the flag fired hundreds of times per bundle on ordinary laptop sleep. multiple_sessions_overlap bounds a session that has no session.end at its last recorded event instead of leaving it open, because treating the ordinary crash signature as "still running" made one power cut overlap every session for the rest of the assignment. That bounding rule no longer lives in the heuristic: it moved to coverage/session-overlap.ts, along with the strict-< overlap test and the contributor comparison below.\n\nThat flag carries the sharpest correction in the group. Its text told the reader an overlap was impossible on a single machine without clock manipulation or log forging, which stopped being true the moment two partners shared a repository: each runs their own recorder, they work at the same time, and the overlap is the assignment being done as assigned. It is now keyed on contributor AND on machine. One verified person’s own two sessions overlapping on ONE machine is the original signal and keeps high severity at 0.95 — and the wording is stronger there than it has ever been, because "they used two machines" is now excluded by evidence rather than conceded in the last sentence. One verified person’s two ENROLLED machines produce no flag: that is D5, a supported flow, and it is reported by the coverage stage as its own fact instead. An overlap where either side is unattributed or unverifiable still fires, because two people is exactly what is unproven there, but at low severity and confidence 0.5 rather than high and 0.95, with detail.unresolvedBy naming which of the three states left it undecided — no_root_key, identity_did_not_verify or no_identity_block, checked in that order because the first is a fact about the DEPLOYMENT rather than about the submission. That arm is the ordinary majority today: one partner unenrolled, neither enrolled, any 1.x bundle, and any deployment with no root key, where every identified session resolves unverifiable and one unset environment variable therefore turned every partner overlap in every bundle into a high-severity accusation. Dropping the finding there would fail toward fewer findings and would let a deployment switch a heuristic off by unsetting a variable, so what moved is the weight it carries into a grader’s triage — high is 8 and low is 1 against the severity weights, so 7.6 becomes 0.5 — and the text now states the overlap, names both readings, refuses in as many words to choose between them, and says what would have settled it. This is deliberately NOT the call bug 13 made for no_session_log, and the difference is the lever: there the severity lever was shared with genuine signature forgery through check 1, so only the text could move, whereas here the partition separates the decidable single-machine case cleanly into its own arm and the undecided arm can be lowered at zero cost to the detection. Two verified, distinct people overlapping produces no flag either — and, for a while, produced nothing at all, which was weaker than intended: the suppression was a bare continue taken before the flag’s detail object existed, and run returns only flags, so the strongest positive evidence of legitimate collaboration in the system was computed and thrown away. coverage/session-overlap.ts now owns the single enumeration of overlapping pairs and the single suppression decision, and returns a three-way partition: the heuristic judges one third, and the coverage stage states the other two as exculpatory facts — "Alice and Bob recorded concurrently for 3h 12m", and separately "one contributor, two machines" — never findings. They cannot drift apart, because none of them computes the suppression: the judged arm’s comparison is typed same_machine-or-unknown, so a suppressed pair is not merely filtered out of the heuristic but unrepresentable there, and re-adding a local suppression is a compile error rather than a silent divergence. The comparison is three-valued for that reason and never a string compare on the contributor key — every unattributed session carries its own singleton key, so comparing keys would read unproven as different people and delete the finding through the back door. A bundle nobody has stamped with contributor verdicts reads as unattributed throughout and behaves exactly as it did before, so no cohort loses findings by not being enrolled. extension_hash_mismatch stays medium for a similar reason: an unrecognised build hash is as likely to mean staff have not published the new release yet as it is to mean a modified recorder.\n\ngap_in_heartbeats also has to answer to the capture policy, and it is the one case where the answer is a scaled threshold rather than a stand-down. Heartbeats are on the floor, but their cadence is tunable, so a course that lengthens the interval would otherwise trip a threshold calibrated for the default. The applied threshold is therefore whichever is larger of the configured floor and ten times the recorded cadence — max rather than replace, so a course that shortens the interval does not get a proportionally tiny threshold and a flood of flags. At the default cadence the arithmetic reproduces the shipped value exactly, so a 1.x bundle is unchanged to the byte.',
    links: [
      {
      ...
      {
        label: 'coverage/coverage-facts.ts',
        href: `${GH}/packages/analysis-core/src/coverage/coverage-facts.ts`,
      },
    ],
  },
  ...
  xscope: {
    title: 'The cross-scope partition',
    body: 'This exists because the flagship collusion surface fired on every honest partner pair, at high severity and confidence 0.95, for every paste. A git-native group submission shares one committed, add-only .provenance/, so Alice’s signed .slog is physically inside Bob’s archive and Bob’s inside Alice’s; the two archives therefore contain the same events, and every paste of at least a hundred characters either partner ever made grouped across both bundles and read out as "shared paste detected across 2 bundles … may indicate content sharing". editing_pattern_clone fired alongside it on a Jaccard of 1.0. Two students doing the assignment exactly as assigned produced the strongest finding the system emits.\n\nThe fix is S20’s same-scope exclusion, and it deliberately mirrors coverage/session-overlap.ts: one module owns the one enumeration and the one suppression decision, and hands out two halves that cannot disagree because they come from a single pass. The heuristics consume lineageOf, an OPAQUE class id, so a heuristic can ask "same scope?" and cannot re-derive or reinterpret the rule; the /compare register consumes exclusions, which names the bundles, how many comparisons were withheld and the commit node keys that established it. The key threaded through the feature extraction is observedCommitKeys, derived once by observedCommitKeysOf and imported by both the browser and the server extractor rather than reimplemented on either side.\n\nThree narrowings, each because the wide reading is worse than the bug. Observed commits only, never witnessed-only: a witnessed-only sha appears solely inside another commit’s parents, which is exactly where a course-issued skeleton’s history lives, so keying on ancestry would put an entire cohort into one lineage and switch cross-submission detection off for the course. The control test — two students who cloned the same starter STILL fire — is the one that dies if anyone widens the key. The ASSUMED_SINGLE_REPOSITORY sentinel never matches on its own, because two unlabelled scopes share a repository key by construction and that is every bundle recorded to date; only a shared COMMIT excludes. The sentinel is TOLERATED but never trusted: a bare sha observed under the sentinel on one side and a real repository key on the other bridges the two, because an unlabelled observation is silent about its repository rather than asserting a different one — but a sha carrying MORE THAN ONE real key builds no bridge at all, since bridging both would merge two distinct repositories through the sentinel in two hops, which is exactly the sha-space merge D12 exists to prevent. And absence is never a match: no observed commits, and a never-computed list, are each their own singleton lineage, so an older construction site behaves exactly as it did and the failure direction is toward comparing rather than toward silence.\n\nNothing persisted changes meaning. globalIdx was not touched, so flags.supporting_seqs and cross_flag_participants.supporting_seqs mean precisely what they meant before.\n\nThe accepted gaps are stated rather than papered over, and they all fail toward more findings. A mid-assignment staff commit pulled by two students while recording is observed by both, so they are excluded as one lineage: a false exclusion, and the price of the key, which is why the visible register is the mitigation. And two partners who never ran a recorded git command have no key at all, so the accusation survives for that pair.',
    invariant:
      'Only a shared OBSERVED commit excludes. Ancestry would put a whole cohort behind one starter into one lineage and switch detection off for the course.',
    links: [
      {
      ...
      {
        label: 'CompareView.tsx',
        href: `${GH}/packages/analyzer/src/views/compare/CompareView.tsx`,
      },
    ],
  },
  cx: {
    title: 'Cross flags and participants',
    body: 'A cross flag names bundles rather than events inside one submission, which is why it is a separate type with its own storage: one row for the finding, one participant row per submission involved. A per-submission flag can hang off a submission id; a finding about a group cannot, and flattening it into per-submission copies would show a reviewer the same finding several times with no way to tell they were one thing.\n\nOn the server the whole semester’s set is deleted and re-inserted on every run, inside one transaction under an advisory lock. Merging is not available: a cross flag’s identity is not stable across runs (adding one submission changes the bundle ids in a group and therefore the flag id) and a fresh run can legitimately produce fewer flags than the last one, which a merge would leave behind as stale rows describing a group that no longer exists.\n\nThe COMPARISON, however, is per assignment, and the two scopes are not the same mistake. The candidate pool was selected on semester and not-superseded and nothing else, so a student’s own hw1 and hw2 were compared against each other, as was every pair of unrelated assignments: a helper carried forward into the next assignment became a high-severity content-sharing finding naming the same student twice, and one person’s kind-stream fingerprint across two assignments clears the clone threshold easily. Submissions are now grouped by assignment and each group is run on its own, with bundles loaded only for the assignments that actually have two submissions. The delete-and-reinsert stays semester-wide because the rows are semester-scoped and stale ones have to go.\n\nPartitioning is more right for a course that reuses a starter across assignments, not less: the reuse is course-issued, so every pair in the cohort would match on it and pooling would turn a staff decision into a semester-wide flag storm. A repeat offender across two assignments is still flagged inside each one, which is where a grader looks. Every positive fixture in the server’s own test seeded its second submission on a DIFFERENT assignment and passed only because no filter existed — the third time in this codebase that the fixtures encoded the thing the code got wrong, so no assertion could have caught it. The seed helper now requires the caller to name the assignment or to say so on purpose.',
    links: [
      {
        label: 'run-cross-heuristics.ts',
        href: `${GH}/packages/analysis-core/src/heuristics/cross/run-cross-heuristics.ts`,
