...
export const nodes: Record<string, ArchNode> = {
  ...
  amf: {
    ...
    body: 'At 2.0 the signature covers eleven fields — course_id, assignment_id, semester, issued_at, files_under_review, ignore, attachments, collaboration, submission, scope and the capture policy — canonicalized as a fresh object with sig and course_cert excluded. All of them are required rather than optional, which is not fussiness: a fixed key set means the Kotlin and Lua ports canonicalize without needing a rule for "which optionals were present", and that rule would be a divergence risk across three hand-written implementations. Unknown top-level keys are ignored for forward compatibility, and because canonicalization names its fields explicitly, an unknown key cannot quietly change the signed bytes.\n\nWhat moved into the signature is the interesting part. course_id is now inside it and must equal the certificate’s, or one course’s key could sign a manifest claiming to be another and both signatures would still verify — that equality is a mandatory conformance vector. policy is inside it because that is the only place it can be: a professor may turn capture down, and a student may not turn it off. ignore and attachments joined files_under_review as three course-signed lists rather than one, and an entry in any of them now takes one of three forms — an exact path, a dir/ prefix, or a *.suffix — matched byte-exact by the single log-core function every recorder re-checks against, because an editor’s own glob is only ever a coarse pre-filter. ignore paths produce no events at all; attachments are sealed into the bundle and hashed but never captured. Manifest has no format_version field at 1.x, so a missing one is defaulted to "1.0" and the file is accepted, never rejected.',
    links: [
  ...
  bman: {
    title: 'manifest.json',
    body: 'Beyond what the box lists, the manifest carries the part check 8 depends on: submission_files, the final on-disk sha256 of every reviewed or attachment path, with files that were absent at seal recorded explicitly as missing with a null hash rather than omitted. A file that vanished and a file that was never listed are different facts, and the manifest keeps them different.\n\nThe file set is discovered rather than read off the manifest: seal walks the workspace, resolves each path’s role against the course-signed scope, and records the ones that come out reviewed or attachment — a dir/ or *.suffix rule cannot be enumerated without looking at the disk. Each submission_files entry therefore carries an optional role of \'reviewed\' or \'attachment\', absent reading as reviewed for every bundle sealed before the field existed. A sibling optional scope_capped flag is set when ANY session the bundle carries reported that its expected-content cap refused an in-scope path — the live session from its own registry, every earlier one from its rolling seal — so a reader can tell "nothing else was in scope" from "the recorder stopped tracking before it saw everything". Whole-bundle, not per-session: a bundle whose first session capped and whose last did not must not read as uncapped, or absence of the flag lets a reader assert "in scope, no activity" from a record the recorder itself called incomplete. The walk itself can drop a path too — one it could not read back, one whose exact entry resolved to a file already sealed under a different spelling, a directory it could not list, a symlink resolving outside the workspace, an in-scope path that IS a symlink and so is never followed at all — and each of those rides in the seal warnings shown to the student rather than vanishing silently; none is a finding by itself.\n\nPer-log hashes rather than one hash over the archive, for a mundane reason: the manifest sits inside the zip it describes and cannot hash itself. Naming each .slog and .slog.meta individually also means a session whose chain failed to validate still appears, with its hash, instead of being quietly dropped: sealing warns and continues. And because these bytes are signed they are frozen: the server strips student source out of stored bundles but never touches manifest.json, which is what keeps an archived bundle verifiable years later.',
    links: [
