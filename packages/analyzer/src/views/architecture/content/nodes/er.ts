...
export const nodes: Record<string, ArchNode> = {
  ...
  students: {
    title: 'students',
    body: 'One row per human, per institution — identity format_version 2.1, and the live identity path. The primary key IS the global student_ref, a uuid defaulted by gen_random_uuid(), so allocating a person and allocating their opaque reference are the same act. The natural key beside it is UNIQUE (institution_id, sso_subject), and sso_subject is the Google sub claim, not the email. Emails get reassigned between people and carry case ambiguity; sub does not, so one human keeps one student_ref across a name change, a department move and a mail migration. Email is still stored, but only as sso_email under a functional index on (institution_id, lower(sso_email)), used to link roster rows after the fact — never as the identity.\n\nThe thing this table exists to fix is a deadlock, not a preference. 2.0 minting required a roster_entries match before it would issue anything, and rosters are populated by Gradescope ingest, which runs only AFTER a student submits. A student therefore could never enrol before their first submission, which is exactly when they need to. A students row is allocatable with no roster row in existence: the mint path does no roster lookup at all, and linking is a separate, later UPDATE that matches on lowered email, is guarded by student_ref IS NULL so it is write-once, and returns zero rows harmlessly when the roster is empty. Ingest runs the same link in the other direction as new roster rows arrive.\n\nNote what is NOT here. There is no institutions table — institution_id is a bare text column with no foreign key anywhere, because its value is read off the signed certificate rather than looked up. And there is no semester_id and no course_id: 2.1 identity sits deliberately outside the course hierarchy, which is why this cluster hangs off nothing above it. One credential serves every course forever.\n\nstudent_pubkey, issued_at, expires_at and issue_count on this row describe only the MOST RECENT issuance and are overwritten on re-enrolment. That is fine for verification, which is a signed artifact consulting no server, and useless for adjudication, which asks a question about history — which is what the separate credentials table is for. The pubkey column is nullable (null before a first issue) and CHECK-constrained to 64 lowercase hex.',
    invariant:
      'Keyed on the SSO subject, never the email — and allocatable before any roster row exists. That second half is what broke the 2.0 deadlock.',
    links: [
      {
      ...
      {
        label: 'mint-credential.ts',
        href: `${GH}/packages/server/src/services/enrollment/mint-credential.ts`,
      },
    ],
  },
  scred: {
    title: 'student_credentials',
    body: 'Every 2.1 credential this server has ever issued, one row per issuance, append-only. It exists because multiple machines per student is a supported first-class flow rather than an edge case: each machine derives its own master secret and therefore its own keypair, signs in under the same account, and receives its own credential over the SAME student_ref. One student legitimately has many public keys, and one row per issuance is the only shape that can say so. No secret copying is needed to use a second machine — export and import are a backup path, not a migration step.\n\nThe question it answers is adjudication’s, not verification’s: "was this key ever issued to this student?" — asked of a bundle recorded on the student’s other machine, possibly years earlier. students.student_pubkey cannot answer it, because it holds only the latest key, so after a second machine enrols the first machine’s key reads as never issued. The lookup is served by an index on (student_ref, student_pubkey), with a reverse index on (institution_id, student_pubkey) for "this bundle carries a key — whose was it?". Both return the records rather than a boolean, because an adjudicator needs the dates.\n\nThere is deliberately NO unique key on (student_ref, student_pubkey), and no issue_count. Two issuances of the same key at different times are two facts, not one fact with a counter, and collapsing them is a smaller version of the overwrite this table exists to stop. The indexes are plain indexes, not unique ones.\n\nIt is audit material, so nothing prunes it, ever. The retention sweep touches only submissions and deletes blobs rather than rows; the session and export purges touch neither this table nor students. The foreign key to students is ON DELETE RESTRICT rather than CASCADE: no path deletes a students row today, and if one is ever written it must fail loudly instead of quietly taking the audit trail with it.\n\nPublic material only. Nothing here would forge anything if it leaked — the private half never leaves the student’s machine, and the server never sees it.',
    invariant:
      'Append-only and never pruned. No unique key on (student_ref, student_pubkey): two issuances are two facts.',
    links: [
      {
  ...
  submissions: {
    title: 'submissions',
    body: 'A resubmission never overwrites anything. version_index is allocated as the current maximum plus one under a FOR UPDATE lock over the existing rows of the same LINEAGE, it is part of the unique key, and the older rows have superseded_by_submission_id pointed at the new one.\n\nThe lineage is version_owner_key, not student_id, and that distinction is load-bearing since migration 0029 made student_id nullable. Postgres treats NULLs as distinct, so a unique key over a nullable student_id stops constraining group rows entirely; and WHERE student_id = NULL is never true, so the FOR UPDATE lock would match zero rows, allocate version 1 for ever, and never form a supersede chain — both silently. version_owner_key is GENERATED ALWAYS by Postgres from the row itself: "student:" || student_id when there is a submitter, "group:" || group_key when there is not, and NULL — which the NOT NULL rejects — when there is neither, so a submission with no lineage is unrepresentable. Being generated, it cannot be written at all, so it can never disagree with the row it describes; and for every row predating 0029 it derives to "student:" || student_id, which is why the new constraint partitions the existing table exactly as the old one did. Every attempt therefore stays queryable, and the cohort list simply filters superseded_by_submission_id IS NULL, which is exactly the predicate on the partial indexes that serve it — two of them now, one keyed for the score sort and one added by migration 0030 for the ingested-at keyset, since the cohort index leads with score_total and cannot serve a chronological page. That self-reference is the one FK Drizzle cannot declare (the table is not built yet at declaration time), so it lives in the migration, ON DELETE SET NULL.\n\nblob_sha256 is the hash of the bundle as the student submitted it, before source stripping, not of the object actually sitting at blob_object_key. It is the dedup key, it is the stable identity of what was handed in, and loadSubmissionIndex borrows it only as a cache key.\n\nThe columns that exist purely so the cohort list can be a single query are flag_counts and top_flags (jsonb denormalizations that the heuristic write path keeps in step with score_total), severity_rank (GENERATED ALWAYS from score_max_severity, so "medium and above" is a range predicate on an indexed integer instead of an OR across strings), and total_active_ms / total_idle_ms (the 60s event-gap Active/Idle pair computeStats already produced, written by computeAndStoreStats at ingest and recompute). Those two time columns are nullable: pre-0021 rows stay NULL until the next ingest or heuristics recompute, and the analyzer renders an em dash. At fifty thousand submissions the two correlated sub-queries the flag columns replaced were the p95.\n\nsource_group_key (migration 0033) is the one column here that exists to stop a false accusation rather than to make a page fast. A Gradescope submitters[] group was, until then, a local variable: the fan-out produced one ingest_files row per co-submitter and what reassembled them into one submission was a coincidence — every row awaited ONE shared rebuild promise, so every row got identical bytes, so blob dedup collapsed them. Any byte divergence at all (scope resolution, entry ordering, the zip writer, a folder partly skipped for no_seal) silently produced two submissions of one repository in one assignment, which is precisely the shape the cross heuristics fire on. The key is <folderKey>/<scopePath> and is carried on both ingest_files and submissions, with a partial UNIQUE (semester_id, source_group_key) WHERE source_group_key IS NOT NULL — partial because the column is null for every non-group path and Postgres would otherwise treat those nulls as distinct anyway. It is keyed on (folder, SCOPE) and never the folder alone, because nested manifest discovery lets one folder yield several scopes and those are different artifacts. It is deliberately a SEPARATE column from group_key, which is the version LINEAGE key and an input to the generated version_owner_key: overloading it would plant a value that is inert today and load-bearing the first time a group: lineage exists.',
    invariant:
      'version_index is allocated under a row lock, over the LINEAGE — (semester, assignment, version_owner_key, version). A resubmission adds a row; it never replaces one.',
    links: [
      {
        label: 'create-submission.ts',
        href: `${GH}/packages/server/src/services/ingest/create-submission.ts`,
      ...
      {
        label: '0033_source_group_key.sql',
        href: `${GH}/packages/server/db/migrations/0033_source_group_key.sql`,
      },
      {
  ...
  subcontrib: {
    title: 'submission_contributors',
    body: 'One group bundle is ONE submissions row with N contributors. Before migration 0029 the same bytes were ingested once per co-submitter into N rows with N duplicated blobs, and this table is what replaced that fan-out. One row per PERSON, never per session.\n\nThe reconciliation is the point. Two sources name contributors — the roster side (the submitter of record, who is named but is not a recording source) and the bundle side (establishBundleContributors, grouped on the verified student_ref) — and a co-submitter who also recorded arrives from both. Two rows for one human would not merely duplicate a name in a list; it would split their score across two apparent people. So a partial unique index on (submission_id, roster_entry_id) WHERE roster_entry_id IS NOT NULL forces them onto one row, and contributor_key carries the bundle-side key verbatim or "roster:<id>" for a submitter nobody could attribute.\n\nWhat deliberately gets no row is as important as what does. An unattributed session — no identity block at all, which is the ordinary and blameless state for almost every bundle today — gets none, and neither does an unverifiable one. analysis-core gives each of those a SINGLETON key precisely because two of them are neither provably one person nor provably two, so a row per session would turn an ordinary five-session solo bundle into five apparent contributors, and would break the sole-contributor scoring rule below. unverifiable is the worse of the two to promote: that block NAMES someone, and honouring it is exactly how a forged identity block would launder work onto the student it names.\n\nScoring is per contributor and per scope, which is the only shape where a grader can act on one partner without implicating the other. A flag is charged to a contributor only when all of its supporting evidence sits in ONE session and that session is attributed; everything else stays scope-level, carried on flags.contributor_key with the empty string as the default. That under-attributes on purpose — a multi-session flag earned by one partner stays at scope level — because under-attribution costs a partner nothing (the finding is still visible at full severity in the roll-up) while over-attribution puts a name on a finding the evidence does not support. The sole-contributor rule is what keeps solo submissions unchanged: with exactly one contributor they are charged everything, scope-level flags included, so their total equals the scope score exactly. Without it every solo student’s rollup score would have silently dropped the day this shipped.\n\nThe stage runs AFTER heuristics, inside the same transaction, on all three write paths through one finalizeContributors. establishBundleContributors mutates the bundle and several heuristics read that stamp, and the ingest path has never stamped it — so stamping earlier would change which flags an ingest produces for any bundle carrying identity, which is a product change, not a schema one. The consequence, noticed and not changed: ingest-time heuristics run on an unstamped bundle while recompute-time ones run on a stamped one, so the two can produce different flags for the same bundle.\n\nTwo concurrency defects hid behind one passing test here. Dedup and submission creation are separate transactions and the worker drains its batch with Promise.all, so two co-submitters carrying byte-identical bundles both cleared dedup before either committed and both created a submission — serial execution looked perfect. That is fixed with a transaction-scoped advisory lock on (semester_id, blob_sha256), the same key dedup uses. The fix then exposed a second: the contributor store pruned any row not in its own set, deleting the partner who had just attached concurrently. It now prunes only attributed rows, which it is the sole author of; a roster row is a fact asserted by the roster side and is not its to remove.\n\nSubmissions that were already fanned out are NOT merged. They stay as N one-contributor rows and read exactly as they do today; merging would rewrite history that flags, cross_flag_participants and ingest_files all point at.',
    invariant:
      'One row per human, never per session. A flag is charged to a contributor only when every supporting event is in one attributed session — under-attribute rather than name the wrong person.',
    links: [
      {
        label: '0029_submission_contributors.sql',
        href: `${GH}/packages/server/db/migrations/0029_submission_contributors.sql`,
      },
      {
        label: 'store-contributors.ts',
        href: `${GH}/packages/server/src/services/contributors/store-contributors.ts`,
      },
      {
        label: 'finalize.ts',
  ...
  xexcl: {
    ...
    links: [
      ...
      {
        label: '0032_exclusion_shared_sessions.sql',
        href: `${GH}/packages/server/db/migrations/0032_exclusion_shared_sessions.sql`,
      },
      {
  ...
  ifiles: {
    title: 'ingest_files',
    body: 'One row per file inside an upload, and the reason a batch can be partially successful. status carries seven values and three of them are not failures: duplicate points at the submission whose bytes it matched, superseded marks a row whose submission a later version replaced, and discarded marks one a staff member resolved away. A partial index on status = \'unmatched\' serves the tray where that resolution happens.\n\nAll three matching references (student, assignment, submission) are ON DELETE SET NULL, so an ingest report degrades to "this file was uploaded" rather than vanishing when a target is removed.\n\nmatch_sid is the Gradescope seam. When it is set, the worker skips the semester’s filename regex and matches straight to the roster by that sid.\n\nIt used to narrow dedup from (semester, blob) to (semester, student, blob) as well, so that two co-submitters of one group bundle each got their own submission out of byte-identical input. That fan-out is gone as of migration 0029: dedup is blob-scoped on every path again, and the second co-submitter is ATTACHED to the existing submission as a contributor instead of producing a second row with a duplicated blob. The student is still preserved — which is all the narrow key was protecting — and their ingest_files row still points at the submission, now with status duplicate and matched_student_id set.\n\nsource_group_key (migration 0033) is what makes that attachment robust rather than lucky. It records the declared group — <folderKey>/<scopePath> — on the row itself, so two co-submitters are known to be one artifact from the metadata rather than inferred from their bytes happening to match. Nullable, because only the Gradescope path declares a group, but CHECKed non-empty when present so an empty string can never masquerade as a group of its own.',
    links: [
