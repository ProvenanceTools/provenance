/**
 * Unit tests for git-repo scope DISCOVERY — pure (bytes in, bytes out), no DB.
 *
 * Moved here with `discoverRepoScopes` itself: discovery is isomorphic and is
 * now shared by the server's ingest pipeline and the analyzer's `/local` route.
 * The server's `repo-scopes.test.ts` keeps the resolution (policy) half.
 *
...
import { selectBundleEntries, zipBundleEntries } from './select-entries.js';
import { discoverRepoScopes, type RepoScope } from './discover-scopes.js';
