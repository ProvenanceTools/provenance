  ...
  '/identity/credential': {
    post: {
      ...
      description: [
        ...
        '',
        'Enrolling again is the SUPPORTED way to set up a second machine: each machine',
        'generates its own keypair and receives its own credential over the one shared',
        'student_ref, so both resolve to a single contributor. machine_count reports how',
        'many distinct keys the student has enrolled and key_first_issued whether this',
        'call added a new one. Every issuance is appended to student_credentials, so the',
        'server can still say a key from an older machine was one of theirs.',
        '',
        'Rate: write.misc.',
      ].join('\n'),
