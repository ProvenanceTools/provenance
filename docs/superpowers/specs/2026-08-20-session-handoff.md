...
| analyzer      | 1313  | ✅                           |
| tools         | 183   | ✅                           |
| provnvim      | 1141  | ✅ gate green                |
| provjet       | 678   | ✅ gate green                |
...
1. **provnvim writer halves** (`peer.observed` + `root_commit_sha`) — its vectors were already
   regenerated by a predecessor and are uncommitted in that repo; they are valid, verify and keep.
2. **provjet writer halves** — same, same vector situation.
3. **Git-spawn portability hardening** (monorepo) — see below.
